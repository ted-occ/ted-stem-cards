...
- **ユーザーガイド** — 初回 Welcome ページ、状況に応じたヒント吹き出し、レベル別ヘルプパネル (H キー)。イベント会場での操作サポート
- **アイドルデモ (アトラクトモード)** — 初期画面で 15 秒操作が無いと自動でボールが動き回るデモを再生。イベント会場での客寄せ用
...
イベント会場で誰も操作していない間、遊び方を自動で見せるデモ機能です。

- 初期画面 (レベル未選択・モーダル/パネルなし) で **15 秒間操作が無い**と自動的に起動
- 盤面中央からボールがランダムに移動・ジャンプを繰り返す (レベルは使用しないフリー移動)
- 画面下部に「デモ再生中」バナーを表示
- 操作 (キーボード・NFC カード・マウス/タッチ) があるまで繰り返し再生
- **どんな操作でも即座に解除**され、ボールは中央へ戻る。解除後は再び 15 秒でデモが再起動
...
app/
  ...
  components/
    ...
    Guide.tsx           # ガイド UI (ヒント吹き出し、ヘルプパネル、Welcome ページ)
  nfc/
...
lib/
  ...
  useGuide.ts           # ユーザーガイド状態管理フック
  guide-content.ts      # ガイドのヒント・ヘルプ内容定義
...
| W | Welcome ページ表示切替 |
| I | 情報オーバーレイ表示切替 |
| J / E / N | 言語切替 (日本語 / English / Español) |
